if RequiredScript == "lib/managers/hudmanagerpd2" then
	...
	function HUDMiniMapJokerEntity:init(parent, key, unit)
		...
		self._avatar:set_color(Color(0, 0, 1))	--tweak_data.contour.character.friendly_color
		self._elevation_indicator:set_color(Color(0, 0, 1))
		self._show_offscreen = true
	end
	...
	HUDMiniMapCivilianEntity = HUDMiniMapCivilianEntity or class(HUDMiniMapEnemyEntity)

	function HUDMiniMapCivilianEntity:init(parent, key, unit)
		HUDMiniMapCivilianEntity.super.init(self, parent, key, unit)
		
		self._avatar:set_color(Color(1, 1, 1))
		self._elevation_indicator:set_color(Color(1, 1, 1))
		self._show_offscreen = true
	end
	
	function HUDMiniMapCivilianEntity:update(...)
		HUDMiniMapCivilianEntity.super.update(self, ...)
...
if RequiredScript == "lib/units/enemies/cop/copbase" then
	...
	local post_init = CopBase.post_init
	
	function CopBase:post_init(...)
	 if managers.hud._hud_minimap then
		managers.hud._hud_minimap:add_entity(HUDMiniMapEnemyEntity, self._unit:key(), self._unit)
		end
...
if RequiredScript == "lib/units/enemies/cop/huskcopbase" then

	local post_init = HuskCopBase.post_init
...
if RequiredScript == "lib/units/civilians/huskcivilianbase" then

	local post_init = HuskCivilianBase.post_init
